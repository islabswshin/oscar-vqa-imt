...
from transformers.pytorch_transformers.modeling_bert import (BertEmbeddings, 
        ...
        BertSelfOutput, BertIntermediate, BertOutput, BertModel,
...
class IMT_Embeddings(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings.
    """
    def __init__(self, config):
        super(IMT_Embeddings, self).__init__()
        ...
        self.IMD_ID = None
        self.IMT_type = None
        self.IMT_embedding = nn.Parameter(torch.FloatTensor(config.hidden_size)) if config.add_imt else None
        ###################################################################################
    ...
    def IMT_init(self,
        ...
        self.IMT_ID = IMT_ID
        self.IMT_type = IMT_type_now

        if is_first_init is True:
            # Initialize Word_Emb
            self.IMT_first_init(SEP_ID)
            if IMT_type_now == "IMT_Emb":
                # Initialize IMT_Emb
                self.IMT_Emb_from_Word_Emb_IMT()
        else:
            if IMT_type_last != IMT_type_now:
                if IMT_type_now == "IMT_Emb":
                    # Get IMT_Emb from Word_Emb
                    self.IMT_Emb_from_Word_Emb_IMT()

                elif IMT_type_now == "Word_Emb":
                    # Get Word_Emb from IMT_Emb
                    self.Word_Emb_IMT_from_IMT_Emb()

            else:
                # If the last and now is the same,
                if IMT_type_now == "Word_Emb":
                    self.IMT_embedding = None

    def IMT_first_init(self, SEP_id):
        self.word_embeddings.weight.data[self.IMT_ID] = self.word_embeddings.weight.data[SEP_id].clone()

    def IMT_Emb_from_Word_Emb_IMT(self):
        # If the last train was trained with word_embedding,
        # and it will be trained with IMT_embedding:
        # Copy IMT_embedding  From word_embedding
        self.IMT_embedding = nn.Parameter(
            self.word_embeddings.weight[self.IMT_ID].clone(),
            requires_grad=True
        )

    def Word_Emb_IMT_from_IMT_Emb(self):
        # If the last train was trained with IMT_embedding,
        # and it will be trained with word_embedding
        # Copy word_embedding From IMT_embedding
        self.word_embeddings.weight.data[self.IMT_ID] = self.IMT_embedding.clone()

    def forward(self, input_ids, token_type_ids=None, position_ids=None):
        seq_length = input_ids.size(1)
        if self.IMT_type == "Word_Emb":
            self.IMT_embedding = None

        if self.IMT_embedding is None:
            words_embeddings = self.word_embeddings(input_ids)
        else:
            words_embeddings = self.word_embeddings(input_ids)
            IMT_idices = (input_ids == self.IMT_ID)
            words_embeddings[IMT_idices] = self.IMT_embedding
...
class BertImgModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.embeddings = IMT_Embeddings(config)
...
class ImageBertForSequenceClassification(BertPreTrainedModel):
    ...
    def IMT_Setup(self):
        if hasattr(self.config, 'add_imt') and self.config.add_imt:
            self.bert.embeddings.IMT_init(
                self.config.IMT_first_init,
                self.config.IMT_type_last,
                self.config.IMT_type_now,
                self.config.special_tokens["[IMT]"],
                self.config.special_tokens["[SEP]"]
            )
