...
from oscar.utils.wandb_setup import wandb_setup
...
def _load_dataset(args, name):
    ...
    if name == 'train':
        ...
        examples = processor.get_test_vqa_2_examples(args.data_dir, 'test-vqa2-qla.json')
    elif name == 'test-dev-vqa2':
        examples = processor.get_test_dev_vqa2_examples(args.data_dir, 'test-dev-vqa2_qla.json')
...
class VQADataset(Dataset):
    ...
    def __init__(self, args, name, tokenizer):
        ...
        assert name in ['train', 'val', 'test-dev2015', 'test2015', 'train+val', 'test-dev-vqa2', 'test-vqa2']
        ...
        # if name == "train":
            # print(f"My rank is {dist.get_rank()}, C1")
            ...
            if args.img_feat_format == 'pt':
                if args.img_feature_dim == 2048: # object features
                    ...
                    if args.use_vg_dev:
                        ...
                        # if name == "train":
                        #     print(f"My rank is {dist.get_rank()}, C2")
                        self.img_features = torch.load(os.path.join(args.data_dir, '{}_img_frcnn_feats.pt'.format(name)))
                        # self.img_features = torch.load(os.path.join(args.data_dir, 'val_img_frcnn_feats.pt'.format(name)))
                        # if name == "train":
    ...
    def tensorize(self, cls_token_at_end=False, pad_on_left=False,
        ...
        for (ex_index, example) in enumerate(self.examples[0: ]):
            ...
            if example.text_b:
                ...
                _truncate_seq_pair(tokens_a, tokens_b, self.args.max_seq_length - 2)
            else:
    ...
    def tensorize_example(self, example, cls_token_at_end=False, pad_on_left=False,
        ...
        if example.text_b:
            if self.imt_token:
                tokens_b = self.tokenizer.tokenize("[IMT]")
                # Account for [CLS], [SEP] with "- 2"
                _truncate_seq_pair(tokens_a, tokens_b, self.args.max_seq_length - 2)
            else:
                tokens_b = self.tokenizer.tokenize(example.text_b)
        ...
        if self.imt_token:
            padding_length = self.args.max_q_length - len(tokens_a)
            tokens = tokens_a
...
def train(args, train_dataset, eval_dataset, model, tokenizer):
    ...
    train_sampler = RandomSampler(train_dataset) if args.n_gpu == 1 else DistributedSampler(train_dataset)
    ...
    if args.scheduler == "constant":
        ...
        logger.info(f"Constant Scheduler ({t_total}) with Warm Up ({args.warmup_steps}) Steps")
    elif args.scheduler == "linear":
        scheduler = WarmupLinearSchedule(optimizer, warmup_steps=args.warmup_steps, t_total=t_total)
        logger.info(f"Linear Scheduler ({t_total}) with Warm Up ({args.warmup_steps}) Steps")
    ...
    logger.info("ㄴNum examples = " + f"{len(train_dataset)}")
    logger.info("ㄴNum Epochs = " + f"{args.num_train_epochs}")
    logger.info("ㄴInstantaneous batch size per GPU = " + f"{args.per_gpu_train_batch_size}")
    logger.info("ㄴGradient Accumulation steps = " + f"{args.gradient_accumulation_steps}")
    logger.info("ㄴTotal train batch size (w. parallel, distributed & accumulation) = " + \
                f"{args.train_batch_size * args.gradient_accumulation_steps * (torch.distributed.get_world_size() if args.n_gpu > 1 else 1)}")
    logger.info("ㄴTotal optimization steps = " + f"{t_total}")
    logger.info("====================================================")

    batch_score, total_score = 0.0, 0.0
    global_step, global_loss = 0, 0.0
    global_batch_score, global_total_score = 0.0, 0.0
    accumulate = 0
    model.zero_grad()
    ...
    #for epoch in train_iterator:
        ...
        if args.adjust_dp and epoch>=3:
            logger.info(f"change droput ratio {args.drop_out} to 0.3")
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            total_score += torch.sum(inputs['labels'])

            global_loss += loss.item()
            global_batch_score += batch_score.item()
            global_total_score += total_score.item()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                logger.info(
                    f"Epoch {epoch} | global_step {global_step}/{t_total} ({((global_step / t_total) * 100):.2f}%) | " \
                    f"global_loss {(global_loss / global_step):.4f} | " \
                    f"global_acc {((global_batch_score / global_total_score) * 100):.4f}% | " \
                    f"lr {(optimizer.param_groups[0]['lr']):.6f} | " \
                    f"global_time {(global_end - global_start):.4f}sec")

                if args.wandb_cfg is not None and is_main_process():
                    wandb.log({
                        "Epoch": epoch,
                        "step": step,
                        ...
                        "global_accuracy": global_batch_score / global_total_score * 100
                    })

                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:# Log metric
                    # synchronize()
        ...
        if (args.local_rank in [-1, 0]) and (args.save_epoch>0 and epoch%args.save_epoch == 0) and (epoch>args.save_after_epoch):
            ...
            while (save_num < 10):
                try:
                    logger.info(f"Saving model attempt: {save_num}")
                    ...
                    save_num += 1
            logger.info(f"Saving model checkpoint {epoch} to {output_dir}")
        ...
        logger.info(f"PROGRESS: {round(100*(epoch + 1) / args.num_train_epochs, 4)}%")
        logger.info(f"EVALERR: {100*best_score}%")

        t_end = time.time()
        logger.info(f"Epoch: {epoch}, Train Time: {(t_end - t_start):.3f}")
    ...
    if args.local_rank in [-1, 0]: # Save the final model checkpoint
        ...
        while (save_num < 10):
            try:
                ...
                save_num += 1
        logger.info(f"Saving the best model checkpoint epoch {best_model['epoch']} to {output_dir}")
...
def evaluate(args, model, eval_dataset=None, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        eval_sampler = SequentialSampler(eval_dataset) if args.n_gpu == 1 else DistributedSampler(eval_dataset)
...
def main():
    ...
    args.n_gpu = get_world_size()
    args.local_rank = local_rank
    args.device = torch.device('cuda', local_rank) if (local_rank >=0 ) else torch.device("cuda:0")
    # synchronize()
    ...
    logger.warning(f"Current cuda device: {torch.cuda.current_device()}")
    set_seed(args.seed, args.n_gpu)

    # Wandb initialize
    if args.wandb_cfg is not None and is_main_process():
        wandb_cfg = json.load(open(args.wandb_cfg))
        _ = wandb_setup(wandb_cfg)

    if args.add_imt:
        if args.do_train:
            args.max_q_length = 27
            args.max_seq_length = 30
    ...
    # print(f"My rank is {local_rank}, device: {args.device}, "
    #       f"dist: {dist.get_world_size(), dist.get_rank()}, "
    #       f"dist: {dist.is_available()} && {dist.is_initialized()}")
    ...
    if args.do_test_vqa2:
        test_vqa2_dataset = VQADataset(args, 'test-vqa2', tokenizer)

    if args.do_test_dev_vqa2:
        test_dev_vqa2_dataset = VQADataset(args, 'test-dev-vqa2', tokenizer)

    # Training
    if args.do_train:
        #train_dataset = load_and_cache_examples(args, args.task_name, tokenizer, evaluate=False)
        # print(f"My rank is {local_rank}, F1")
        train_dataset = VQADataset(args, 'train', tokenizer)
        # print(f"My rank is {local_rank}, F2")
    ...
    if args.do_train and (args.n_gpu > 1 or torch.distributed.get_rank() == 0):
    ...
    # Test-Vqa2
    if args.do_test_vqa2 and args.local_rank in [-1, 0]:
        checkpoints = [args.output_dir]
        if args.eval_all_checkpoints:
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint)
            model.to(args.device)
            test(args, model, test_vqa2_dataset, prefix=global_step)

    # Test-Dev-Vqa2
    if args.do_test_dev_vqa2 and args.local_rank in [-1, 0]:
        checkpoints = [args.output_dir]
        if args.eval_all_checkpoints:
            checkpoints = list(os.path.dirname(c) for c in
                               sorted(glob.glob(args.output_dir + '/**/' + WEIGHTS_NAME, recursive=True)))
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint)
            model.to(args.device)
            test(args, model, test_dev_vqa2_dataset, prefix=global_step)
