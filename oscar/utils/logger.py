...
def setup_loguru_logger(name, save_dir, distributed_rank, filename="log.txt"):
    # don't log results for the non-master process
    if distributed_rank > 0:
        return nologger()
    ...
    if save_dir:
        logger.add(filename, format="{time:YYYY-MM-DD at HH:mm:ss} | {level} | {message}")
